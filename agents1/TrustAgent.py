...
from actions1.CustomActions import Idle, CarryObject, Drop
...
class TrustAgent(BaselineAgent):
    ...
    def decide_on_actions(self, state):
        ...
        for info in state.values():
            if 'is_human_agent' in info and self._humanName in info['name'] and len(
                ...
                for vic in remainingVics:
                    ...
                    if vic in self._foundVictims and vic in self._todo and len(self._searchedRooms) == 0:
                        ...
                        self._sendMessage('Moving to ' + self._foundVictimLocs[vic][
                            'room'] + ' to pick up ' + self._goalVic + '. Please come there as well to help me carry ' + self._goalVic + ' to the drop zone.',
                                          'RescueBot')
                ...
                for info in state.values():
                    if 'class_inheritance' in info and 'ObstacleObject' in info['class_inheritance'] and 'rock' in info[
                        ...
                        if self._door['room_name'] not in self._tosearch:
                            if self.get_trust(self._folder)[self._humanName][competence] < -0.5:
                                self._answered = True
                                self._waiting = False
                                # Add area to the to do list
                                self._tosearch.append(self._door['room_name'])
                                self._phase = Phase.FIND_NEXT_GOAL
                            elif self.get_trust(self._folder)[self._humanName][competence] < 0.5 and self._distanceHuman == 'far':
                                self._answered = True
                                self._waiting = False
                                # Add area to the to do list
                                self._tosearch.append(self._door['room_name'])
                        ...
                        # Remain idle untill the human communicates what to do with the identified obstacle
                            ...
                            if self.get_trust(self._folder)[self._humanName][willingness] < 0.5 and self._idle_timer > Punishment.MEDIUM_WILLINGNESS_TIMEOUT:
                                self._answered = True
                                self._waiting = False
                                # Add area to the to do list
                                self._tosearch.append(self._door['room_name'])
                                self._phase = Phase.FIND_NEXT_GOAL
                            elif self.get_trust(self._folder)[self._humanName][willingness] < -0.5 and self._idle_timer > Punishment.LOW_WILLINGNESS_TIMEOUT:
                                self._answered = True
                                self._waiting = False
                                # Add area to the to do list
                                self._tosearch.append(self._door['room_name'])
